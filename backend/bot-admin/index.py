...
GET  /moderators?user_id=&session_token=    — список модераторов (только админ).
POST /moderators                            — добавить/удалить модератора (только админ).
GET  /support?user_id=&session_token=       — очередь запросов помощи (модератор/админ).
POST /support                               — действия с запросами (assign/close/reply).
"""
import json
import os
import re
...
# ── БД helpers для модераторов и support ─────────────────────────
...
def is_moderator(conn, user_id: int) -> bool:
    with conn.cursor() as cur:
        cur.execute(f"SELECT 1 FROM {SCHEMA}.moderators WHERE user_id = %s", (user_id,))
        return bool(cur.fetchone())

def is_mod_or_admin(conn, user_id: int, session_token: str) -> bool:
    if verify_admin(conn, user_id, session_token):
        return True
    return is_moderator(conn, user_id)

def get_moderators(conn) -> list:
    with conn.cursor() as cur:
        cur.execute(f"""
            SELECT m.user_id, m.username, m.assigned_by, m.assigned_at
            FROM {SCHEMA}.moderators m
            ORDER BY m.assigned_at DESC
        """)
        return [{"user_id": r[0], "username": r[1], "assigned_by": r[2],
                 "assigned_at": str(r[3])} for r in cur.fetchall()]

def add_moderator_db(conn, target_user_id: int, username: str, assigned_by: int):
    with conn.cursor() as cur:
        cur.execute(f"""
            INSERT INTO {SCHEMA}.moderators (user_id, username, assigned_by)
            VALUES (%s, %s, %s)
            ON CONFLICT (user_id) DO UPDATE SET username = EXCLUDED.username, assigned_by = EXCLUDED.assigned_by
        """, (target_user_id, username, assigned_by))
    conn.commit()

def remove_moderator_db(conn, target_user_id: int):
    with conn.cursor() as cur:
        cur.execute(f"DELETE FROM {SCHEMA}.moderators WHERE user_id = %s", (target_user_id,))
    conn.commit()

def get_all_mod_ids(conn) -> list:
    with conn.cursor() as cur:
        cur.execute(f"SELECT user_id FROM {SCHEMA}.moderators")
        return [r[0] for r in cur.fetchall()]

def save_support_request(conn, user_id: int, username: str,
                          request_type: str, description: str) -> int:
    with conn.cursor() as cur:
        cur.execute(f"""
            INSERT INTO {SCHEMA}.support_requests
                (user_id, username, request_type, description, status, created_at)
            VALUES (%s, %s, %s, %s, 'new', NOW())
            RETURNING id
        """, (user_id, username, request_type, description[:2000]))
        new_id = cur.fetchone()[0]
    conn.commit()
    return new_id

def get_support_queue(conn, status_filter: str = None) -> list:
    with conn.cursor() as cur:
        if status_filter:
            cur.execute(f"""
                SELECT id, user_id, username, request_type, description,
                       status, assigned_moderator, created_at, resolved_at
                FROM {SCHEMA}.support_requests
                WHERE status = %s
    ...
    return [{
        ...
        "request_type": r[3], "description": r[4],
        "status": r[5], "assigned_moderator": r[6],
        "created_at": str(r[7]), "resolved_at": str(r[8]) if r[8] else None,
    } for r in rows]

def set_dialog_state(conn, user_id: int, state: str, payload: str = ""):
    with conn.cursor() as cur:
        cur.execute(f"""
            INSERT INTO {SCHEMA}.bot_dialog_state (user_id, state, payload, updated_at)
            VALUES (%s, %s, %s, NOW())
            ON CONFLICT (user_id) DO UPDATE SET state = EXCLUDED.state, payload = EXCLUDED.payload, updated_at = NOW()
        """, (user_id, state, payload))
    conn.commit()

def get_dialog_state(conn, user_id: int) -> dict:
    with conn.cursor() as cur:
        cur.execute(f"SELECT state, payload FROM {SCHEMA}.bot_dialog_state WHERE user_id = %s", (user_id,))
        row = cur.fetchone()
    return {"state": row[0], "payload": row[1] or ""} if row else {"state": "", "payload": ""}

def clear_dialog_state(conn, user_id: int):
    with conn.cursor() as cur:
        cur.execute(f"DELETE FROM {SCHEMA}.bot_dialog_state WHERE user_id = %s", (user_id,))
    conn.commit()

def count_user_requests_today(conn, user_id: int) -> int:
    with conn.cursor() as cur:
        cur.execute(f"""
            SELECT COUNT(*) FROM {SCHEMA}.support_requests
...
def wh_callback(conn, payload_str: str, cb_id: str, chat_id: int, user_id: int):
    ...
    elif action == "help_type":
        req_type = value
        target_uid = int(pl.split(":")[-1]) if pl.count(":") >= 2 else user_id
        set_dialog_state(conn, target_uid, "awaiting_help_desc", req_type)
        type_ru = REQUEST_TYPE_RU.get(req_type, req_type)
        answer_cb(cb_id, f"✅ {type_ru}")
        send_max(tcid,
            f"📝 Тип запроса: {type_ru}\n\nОпишите вашу проблему текстом — я передам её модераторам."
        )
...
GUIDE_TEXT = """📖 Инструкция по Страж-боту

1️⃣ Начало работы
• Добавьте бота в канал/группу и назначьте его администратором
• Напишите /start — бот предложит выбрать зоны модерации
• Запустите команду в личном чате для начала пробного периода

2️⃣ Настройка модерации
• /moderation — интерактивное меню: включить/выключить, выбрать зоны (Канал / Группа / Личный чат)
• /add_word [слово] — добавить запрещённое слово
• /remove_word [слово] — удалить слово из фильтра
• Бот автоматически удаляет сообщения с запрещёнными словами, ссылками и флудом

3️⃣ Просмотр истории
• /log — последние нарушения за 24 часа в текущем чате

4️⃣ Запрос помощи
• /help — интерактивная форма: выберите тип запроса → опишите проблему
• Ваш запрос получат модераторы и ответят в личном чате

5️⃣ Управление модераторами (только владелец)
• /add_moderator [user_id] — назначить модератора
• /remove_moderator [user_id] — снять модератора
• /moderators_list — список текущих модераторов

6️⃣ Панель модератора
• /support_queue — очередь запросов помощи
• /assign_request [ID] — взять запрос в работу
• /close_request [ID] — закрыть запрос
• /reply_to_user [ID] [текст] — ответить пользователю

7️⃣ Передача прав
• Через веб-кабинет: раздел «Настройки» → «Передать права»

❓ Часто задаваемые вопросы
Q: Бот не удаляет сообщения?
A: Убедитесь, что бот — администратор чата с правом удалять сообщения, и модерация включена (/moderation).

Q: Как добавить несколько слов сразу?
A: Используйте /add_word для каждого слова по отдельности или добавьте через веб-кабинет.

Q: Пробный период не запускается?
A: Напишите боту /start в личном чате.
"""
...
def wh_guide(chat_id: int):
    send_max(chat_id, GUIDE_TEXT)

def wh_help_cmd(conn, chat_id: int, user_id: int):
    set_dialog_state(conn, user_id, "awaiting_help_type")
    send_max(chat_id,
        "🆘 Запрос помощи\n\nВыберите тип запроса:",
        [[
            {"type": "callback", "text": "🔧 Техническая проблема", "payload": f"help_type:technical:{user_id}"},
            {"type": "callback", "text": "❓ Вопрос по функционалу", "payload": f"help_type:feature:{user_id}"},
        ], [
            {"type": "callback", "text": "🚫 Жалоба на контент",    "payload": f"help_type:complaint:{user_id}"},
            {"type": "callback", "text": "📝 Другое",               "payload": f"help_type:other:{user_id}"},
        ]]
    )

def wh_add_moderator(conn, chat_id: int, args: str, caller_user_id: int):
    target = args.strip()
    if not target:
        send_max(chat_id, "Использование: /add_moderator [user_id]\nПример: /add_moderator 123456789")
        return
    if not is_owner(conn, caller_user_id):
        send_max(chat_id, "⛔ Только владелец бота может назначать модераторов.")
        return
    try:
        target_id = int(target.lstrip("@"))
    except ValueError:
        send_max(chat_id, "⚠️ Укажите числовой user_id пользователя.")
        return
    add_moderator_db(conn, target_id, target, caller_user_id)
    send_max(chat_id, f"✅ Пользователь {target} назначен модератором.")
    send_max(target_id,
        "🎖 Вы назначены модератором бота Страж.\n\n"
        "Доступные команды:\n"
        "• /support_queue — очередь запросов\n"
        "• /assign_request [ID] — взять в работу\n"
        "• /close_request [ID] — закрыть\n"
        "• /reply_to_user [ID] [текст] — ответить пользователю"
    )

def wh_remove_moderator(conn, chat_id: int, args: str, caller_user_id: int):
    target = args.strip()
    if not target:
        send_max(chat_id, "Использование: /remove_moderator [user_id]")
        return
    if not is_owner(conn, caller_user_id):
        send_max(chat_id, "⛔ Только владелец бота может снимать модераторов.")
        return
    try:
        target_id = int(target.lstrip("@"))
    except ValueError:
        send_max(chat_id, "⚠️ Укажите числовой user_id.")
        return
    remove_moderator_db(conn, target_id)
    send_max(chat_id, f"✅ Пользователь {target} снят с роли модератора.")

def wh_moderators_list(conn, chat_id: int, caller_user_id: int):
    if not is_owner(conn, caller_user_id):
        send_max(chat_id, "⛔ Только владелец может просматривать список модераторов.")
        return
    mods = get_moderators(conn)
    if not mods:
        send_max(chat_id, "👥 Модераторов пока нет.\nНазначьте: /add_moderator [user_id]")
        return
    lines = ["👥 Список модераторов:\n"]
    for m in mods:
        lines.append(f"• ID: {m['user_id']} ({m['username'] or '—'})")
    send_max(chat_id, "\n".join(lines))

def wh_support_queue(conn, chat_id: int, caller_user_id: int):
    if not is_moderator(conn, caller_user_id) and not is_owner(conn, caller_user_id):
        send_max(chat_id, "⛔ Доступ только для модераторов и владельца.")
        return
    queue = get_support_queue(conn)
    if not queue:
        send_max(chat_id, "✅ Очередь запросов пуста. Все запросы обработаны.")
        return
    lines = [f"📋 Очередь запросов ({len(queue)}):\n"]
    for r in queue:
        tp = REQUEST_TYPE_RU.get(r["request_type"], r["request_type"])
        lines.append(
            f"ID: {r['id']} | @{r['username'] or r['user_id']} | {tp} | {r['status'].upper()}\n"
            f"   {r['description'][:80]}…"
        )
    send_max(chat_id, "\n".join(lines))

def wh_assign_request(conn, chat_id: int, args: str, caller_user_id: int):
    if not is_moderator(conn, caller_user_id) and not is_owner(conn, caller_user_id):
        send_max(chat_id, "⛔ Доступ только для модераторов.")
        return
    try:
        req_id = int(args.strip())
    except ValueError:
        send_max(chat_id, "Использование: /assign_request [ID]")
        return
    with conn.cursor() as cur:
        cur.execute(f"""
            UPDATE {SCHEMA}.support_requests
            SET status = 'in_progress', assigned_moderator = %s
            WHERE id = %s AND status = 'new'
            RETURNING user_id, username, request_type
        """, (caller_user_id, req_id))
        row = cur.fetchone()
    conn.commit()
    if not row:
        send_max(chat_id, f"⚠️ Запрос #{req_id} не найден или уже в работе.")
        return
    send_max(chat_id, f"✅ Запрос #{req_id} взят в работу.")
    send_max(row[0],
        f"ℹ️ Ваш запрос #{req_id} «{REQUEST_TYPE_RU.get(row[2], row[2])}» взят в обработку модератором."
    )

def wh_close_request(conn, chat_id: int, args: str, caller_user_id: int):
    if not is_moderator(conn, caller_user_id) and not is_owner(conn, caller_user_id):
        send_max(chat_id, "⛔ Доступ только для модераторов.")
        return
    try:
        ...
        send_max(chat_id, "Использование: /close_request [ID]")
        return
    with conn.cursor() as cur:
        cur.execute(f"""
            UPDATE {SCHEMA}.support_requests
    ...
    conn.commit()
    if not row:
        send_max(chat_id, f"⚠️ Запрос #{req_id} не найден.")
        return
    send_max(chat_id, f"✅ Запрос #{req_id} закрыт.")
    send_max(row[0], f"✅ Ваш запрос #{req_id} закрыт модератором.")

def wh_reply_to_user(conn, chat_id: int, args: str, caller_user_id: int):
    if not is_moderator(conn, caller_user_id) and not is_owner(conn, caller_user_id):
        send_max(chat_id, "⛔ Доступ только для модераторов.")
        return
    parts = args.strip().split(None, 1)
    if len(parts) < 2:
        send_max(chat_id, "Использование: /reply_to_user [ID] [текст ответа]")
        return
    try:
        req_id = int(parts[0])
    except ValueError:
        send_max(chat_id, "⚠️ Неверный ID запроса.")
        return
    reply_text = parts[1]
    with conn.cursor() as cur:
        cur.execute(f"""
    ...
    send_max(chat_id, f"✅ Ответ отправлен пользователю @{row[1] or row[0]}. Запрос #{req_id} закрыт.")
...
def handle_webhook(event: dict, conn) -> dict:
    ...
    if upd_type == "message_callback":
        ...
        if text.startswith("/start"):
            ct = max_get_chat(cid)
            wh_start(conn, cid, uid, ct)
        elif text.startswith("/moderation"):
            wh_mod_cmd(conn, cid, uid)
        elif text.startswith("/add_word"):
            wh_add_word(conn, cid, text[9:].strip(), uid)
        elif text.startswith("/remove_word"):
            wh_remove_word(conn, cid, text[12:].strip())
        elif text.startswith("/log"):
            wh_log_cmd(conn, cid)
        elif text.startswith("/guide") or text.startswith("/инструкция"):
            wh_guide(cid)
        elif text.startswith("/help") or text.startswith("/помощь"):
            wh_help_cmd(conn, cid, uid)
        elif text.startswith("/add_moderator"):
            wh_add_moderator(conn, cid, text[14:].strip(), uid)
        elif text.startswith("/remove_moderator"):
            wh_remove_moderator(conn, cid, text[17:].strip(), uid)
        elif text.startswith("/moderators_list"):
            wh_moderators_list(conn, cid, uid)
        elif text.startswith("/support_queue"):
            wh_support_queue(conn, cid, uid)
        elif text.startswith("/assign_request"):
            wh_assign_request(conn, cid, text[15:].strip(), uid)
        elif text.startswith("/close_request"):
            wh_close_request(conn, cid, text[14:].strip(), uid)
        elif text.startswith("/reply_to_user"):
            wh_reply_to_user(conn, cid, text[14:].strip(), uid)
        else:
            ds = get_dialog_state(conn, uid)
            if ds["state"] == "awaiting_help_desc" and text.strip():
                req_type = ds["payload"] or "other"
                if count_user_requests_today(conn, uid) >= 5:
                    send_max(cid, "⚠️ Лимит запросов: не более 5 в сутки.")
                else:
                    uname = sender.get("username", "") or sender.get("name", "")
                    req_id = save_support_request(conn, uid, uname, req_type, text.strip())
                    clear_dialog_state(conn, uid)
                    type_ru = REQUEST_TYPE_RU.get(req_type, req_type)
                    send_max(cid,
                        f"✅ Ваш запрос #{req_id} принят.\n"
                        f"Тип: {type_ru}\n"
                        f"Модераторы рассмотрят его и ответят в личном чате."
                    )
                    mod_ids = get_all_mod_ids(conn)
                    for mid in mod_ids:
                        send_max(mid,
                            f"🔔 Новый запрос помощи #{req_id}\n"
                            f"От: @{uname or uid}\n"
                            f"Тип: {type_ru}\n"
                            f"Описание: {text[:200]}\n\n"
                            f"/assign_request {req_id} — взять в работу"
                        )
            else:
...
def handler(event: dict, context) -> dict:
    ...
    try:
        ...
        if "moderators" in path:
            ...
            token   = params.get("session_token", "") or (json.loads(event.get("body") or "{}")).get("session_token", "")
            if not user_id:
                return {"statusCode": 400, "headers": CORS, "body": json.dumps({"error": "user_id required"})}
            if not verify_admin(conn, int(user_id), token):
                return {"statusCode": 403, "headers": CORS, "body": json.dumps({"error": "Доступ запрещён"})}
            ...
            if method == "GET":
                mods = get_moderators(conn)
                return {"statusCode": 200, "headers": CORS, "body": json.dumps({"moderators": mods})}

            if method == "POST":
                body          = json.loads(event.get("body") or "{}")
                action        = body.get("action", "add")
                target_uid    = body.get("target_user_id")
                target_uname  = body.get("target_username", "")
                if not target_uid:
                    return {"statusCode": 400, "headers": CORS, "body": json.dumps({"error": "target_user_id required"})}
                if action == "add":
                    add_moderator_db(conn, int(target_uid), target_uname, int(user_id))
                    return {"statusCode": 200, "headers": CORS, "body": json.dumps({"ok": True})}
                if action == "remove":
                    remove_moderator_db(conn, int(target_uid))
                    return {"statusCode": 200, "headers": CORS, "body": json.dumps({"ok": True})}
                return {"statusCode": 400, "headers": CORS, "body": json.dumps({"error": "action must be add or remove"})}
        ...
        if "support" in path:
            ...
            user_id  = params.get("user_id") or body_raw.get("user_id")
            token    = params.get("session_token", "") or body_raw.get("session_token", "")
            if not user_id:
                return {"statusCode": 400, "headers": CORS, "body": json.dumps({"error": "user_id required"})}
            if not is_mod_or_admin(conn, int(user_id), token):
                return {"statusCode": 403, "headers": CORS, "body": json.dumps({"error": "Доступ запрещён"})}

            if method == "GET":
                status_filter = params.get("status")
                queue = get_support_queue(conn, status_filter)
                return {"statusCode": 200, "headers": CORS,
                        "body": json.dumps({"requests": queue, "total": len(queue)})}

            if method == "POST":
                action = body_raw.get("action", "")
                req_id = body_raw.get("request_id")
                if not req_id:
                    return {"statusCode": 400, "headers": CORS, "body": json.dumps({"error": "request_id required"})}
                ...
                if action == "assign":
                    ...
                    conn.commit()
                    if not row:
                        return {"statusCode": 404, "headers": CORS, "body": json.dumps({"error": "Запрос не найден или уже в работе"})}
                    return {"statusCode": 200, "headers": CORS, "body": json.dumps({"ok": True})}
                ...
                if action == "reply":
                    reply_text = body_raw.get("reply_text", "").strip()
                    if not reply_text:
                        return {"statusCode": 400, "headers": CORS, "body": json.dumps({"error": "reply_text required"})}
                    with conn.cursor() as cur:
                        cur.execute(f"""
                            UPDATE {SCHEMA}.support_requests
                            SET status = 'resolved', resolved_at = NOW()
                            WHERE id = %s RETURNING user_id, username
                        """, (int(req_id),))
                        row = cur.fetchone()
                    conn.commit()
                    if not row:
                        return {"statusCode": 404, "headers": CORS, "body": json.dumps({"error": "Запрос не найден"})}
                    send_max(row[0], f"📨 Ответ на ваш запрос #{req_id}:\n\n{reply_text}\n\n✅ Запрос закрыт.")
                    return {"statusCode": 200, "headers": CORS, "body": json.dumps({"ok": True})}

                return {"statusCode": 400, "headers": CORS, "body": json.dumps({"error": "action must be assign/close/reply"})}
